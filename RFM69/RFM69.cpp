...
//Davis public variables
	...
	static volatile uint8_t RFM69::Davis_DATA[DAVIS_PACKET_LEN];  // recv/xmit buf, including header, CRC
    //static volatile uint8_t RFM69::Davis_mode; //should be protected?
    static volatile bool RFM69::_packetReceived;
    static volatile uint8_t RFM69::CHANNEL;
    static volatile int16_t RFM69::Davis_RSSI;
	
    static volatile bool RFM69::Davis_Switch;


bool RFM69::initialize(uint8_t freqBand , uint8_t nodeID, uint8_t networkID, bool Switch) //initialize for DavisRFM69
{
  ...
  attachInterrupt(_interruptNum, RFM69::Davis_isr0, RISING);
...
bool RFM69::initialize(uint8_t nodeID)  //initialize for RFM69
{
	 Davis_Switch = true;
  const uint8_t CONFIG[][2] =
  {
    ...
    /* 0x2F */ { REG_SYNCVALUE1, 0xcb },      // attempt to make this compatible with sync1 byte of RFM12B lib
    /* 0x30 */ { REG_SYNCVALUE2, 0x89 }, // NETWORK ID
...
//Davis functions definitions 
uint8_t RFM69::reverseBits(uint8_t b)
{
...
bool RFM69::Davis_receiveDone() {
  return _packetReceived;
}


void RFM69::Davis_interruptHandler() {
	if(Davis_Switch) 
	{
		interruptHandler();
		return;
	}
	uint8_t length = readReg(REG_PAYLOADLENGTH);
    writeReg(REG_PAYLOADLENGTH,DAVIS_PACKET_LEN);
  RSSI = readRSSI();  // Read up front when it is most likely the carrier is still up
  if (_mode == RF69_MODE_RX && (readReg(REG_IRQFLAGS2) & RF_IRQFLAGS2_PAYLOADREADY))
  {
    setMode(RF69_MODE_STANDBY);
    Davis_select();   // Select RFM69 module, disabling interrupts
    SPI.transfer(REG_FIFO & 0x7f);
    for (uint8_t i = 0; i < DAVIS_PACKET_LEN; i++) Davis_DATA[i] = reverseBits(SPI.transfer(0));

    _packetReceived = true;
    writeReg(REG_PAYLOADLENGTH,length);
    Davis_unselect();  // Unselect RFM69 module, enabling interrupts
  }
}


void RFM69::Davis_select() {
  noInterrupts();
  // Save current SPI settings
  _SPCR = SPCR;
  _SPSR = SPSR;
...
void RFM69::Davis_setFrequency(uint32_t FRF)
{
...
uint16_t RFM69::Davis_crc16_ccitt(volatile uint8_t *buf, uint8_t len, uint16_t initCrc)
{
...
void RFM69::Davis_setChannel(uint32_t channel)
{
  uint8_t oldMode = _mode;
  if (oldMode == RF69_MODE_TX) {
    setMode(RF69_MODE_RX);
...
void RFM69::Davis_isr0() { _inISR = true; selfPointer->Davis_interruptHandler(); _inISR = false; }

void RFM69::Davis_hop()
{
	Davis_setChannel(++CHANNEL);
}

void RFM69::Davis_unselect()
{
	digitalWrite(_slaveSelectPin, HIGH);
  // Restore SPI settings to what they were before talking to RFM69
  SPCR = _SPCR;
